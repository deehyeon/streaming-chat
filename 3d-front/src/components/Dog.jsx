...
// export default를 붙여줍니다.
export default function Dog({ position, color, onClick, message }) {
    const dogRef = useRef();
        ...
        {hovered && (
          <Html position={[0, 1.3, 0]} center distanceFactor={8}>
            <div style={{
              ...
              padding: '8px 12px',
              borderRadius: '12px',
              border: `2px solid ${color}`, // 강아지 색상으로 테두리
              color: '#333',
              fontSize: '14px',
              fontWeight: 'bold',
              whiteSpace: 'nowrap',
              boxShadow: '0 4px 6px rgba(0,0,0,0.1)',
              pointerEvents: 'none', // 말풍선이 클릭을 방해하지 않도록
              transform: 'translate3d(0,0,0)', // 텍스트 깨짐 방지
            }}>
              {message}
              {/* 말풍선 꼬리 (CSS로 만듦) */}
              <div style={{
